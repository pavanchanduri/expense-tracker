...
def app(tmp_path, monkeypatch):
    """
    Flask app wired to a temp-file SQLite DB so the test client and
    direct DB helpers share the same physical file.  init_db() is called
    fresh for every test — the real expense_tracker.db is never touched.
    """
...
class TestInsertExpenseHelper:
    ...
    def test_insert_expense_returns_positive_integer_id(self, app, db_conn, test_user_id):
    ...
    def test_insert_expense_row_queryable_after_insert(self, app, db_conn, test_user_id):
        ...
        assert row is not None, (
            "Expected the row to be queryable by its returned id"
        )
    ...
    def test_insert_expense_stores_correct_user_id(self, app, db_conn, test_user_id):
        ...
        with app.app_context():
            row_id = insert_expense(test_user_id, 50.0, "Food", "2026-03-20", "Lunch")
        row = db_conn.execute("SELECT user_id FROM expenses WHERE id = ?", (row_id,)).fetchone()
    ...
    def test_insert_expense_stores_correct_amount(self, app, db_conn, test_user_id):
        ...
        with app.app_context():
            row_id = insert_expense(test_user_id, 50.0, "Food", "2026-03-20", "Lunch")
        row = db_conn.execute("SELECT amount FROM expenses WHERE id = ?", (row_id,)).fetchone()
        assert pytest.approx(row["amount"]) == 50.0, (
    ...
    def test_insert_expense_stores_correct_category(self, app, db_conn, test_user_id):
        ...
        with app.app_context():
            row_id = insert_expense(test_user_id, 50.0, "Food", "2026-03-20", "Lunch")
        row = db_conn.execute("SELECT category FROM expenses WHERE id = ?", (row_id,)).fetchone()
    ...
    def test_insert_expense_stores_correct_date(self, app, db_conn, test_user_id):
        ...
        with app.app_context():
            row_id = insert_expense(test_user_id, 50.0, "Food", "2026-03-20", "Lunch")
        row = db_conn.execute("SELECT date FROM expenses WHERE id = ?", (row_id,)).fetchone()
        assert row["date"] == "2026-03-20", (
            f"Expected date '2026-03-20', got {row['date']}"
        )

    def test_insert_expense_stores_correct_description(self, app, db_conn, test_user_id):
        """Spec: Inserted row must store the supplied description string."""
        with app.app_context():
            row_id = insert_expense(test_user_id, 50.0, "Food", "2026-03-20", "Lunch")
        row = db_conn.execute("SELECT description FROM expenses WHERE id = ?", (row_id,)).fetchone()
        assert row["description"] == "Lunch", (
            f"Expected description 'Lunch', got {row['description']}"
        )

    def test_insert_expense_description_none_stores_sql_null(self, app, db_conn, test_user_id):
    ...
    def test_insert_expense_sequential_calls_produce_unique_ids(
        ...
        with app.app_context():
            ...
            id2 = insert_expense(test_user_id, 200.0, "Health", "2026-03-23", "Pharmacy")
        assert id1 != id2, "Each insert_expense call must produce a unique row id"
        assert _expense_count(db_conn, test_user_id) == 2, (
    ...
    def test_get_categories_tuple_has_exactly_7_entries(self, auth_client):
        ...
        for expected in (
            ...
            assert expected in CATEGORIES, (
                f"'{expected}' must be in CATEGORIES"
            )
...
class TestPostAddExpenseHappyPath:
    ...
    def test_post_valid_inserts_exactly_one_row(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_valid_stores_correct_category(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_valid_stores_correct_description(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_valid_stores_correct_user_id(self, auth_client, db_conn, test_user_id):
...
class TestPostAddExpenseBlankDescription:
    ...
    def test_post_blank_description_inserts_row(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_blank_description_stores_null(self, auth_client, db_conn, test_user_id):
...
class TestPostAddExpenseAmountValidation:
    ...
    def test_post_missing_amount_no_row_inserted(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_zero_amount_rerenders_form(self, auth_client):
        ...
        response = auth_client.post("/expenses/add", data={**_VALID_FORM, "amount": "0"})
    ...
    def test_post_negative_amount_rerenders_form(self, auth_client):
        ...
        response = auth_client.post("/expenses/add", data={**_VALID_FORM, "amount": "-10"})
    ...
    def test_post_negative_amount_no_row_inserted(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_non_numeric_amount_no_row_inserted(
        ...
        assert _expense_count(db_conn, test_user_id) == 0, (
...
class TestPostAddExpenseCategoryValidation:
    ...
    def test_post_invalid_category_no_row_inserted(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_empty_category_no_row_inserted(self, auth_client, db_conn, test_user_id):
    ...
    def test_post_sql_injection_in_category_rejected(self, auth_client, db_conn, test_user_id):
        ...
        count = db_conn.execute("SELECT COUNT(*) AS n FROM expenses").fetchone()["n"]
        assert isinstance(count, int), "expenses table must still be intact after SQL injection attempt"
